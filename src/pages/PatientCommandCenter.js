            ...
            <div className="section-card">
              ...
              {consultations.length === 0 ? (
                ...
                consultations.map((visit) => (
                  <div key={visit.id} className="section-card" style={{ marginBottom: "14px", padding: "16px" }}>
                    ...
                    <div style={{ marginTop: "10px" }}>
                      ...
                      {visit.medicalRecords?.length ? (
                        <div style={{ marginTop: "8px" }}>
                          {visit.medicalRecords.map((record) => (
                            <div key={record.recordId} className="alert alert-info" style={{ marginBottom: "8px" }}>
                              <div style={{ width: "100%" }}>
                                <p style={{ margin: 0, fontWeight: 600 }}>{record.fileName} ({record.fileType})</p>
                                <p style={{ margin: "4px 0" }}>{record.fileSummary}</p>
                                <small>{formatApiDateTime(record.uploadedAt)}</small>
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          {tab === "reports" && (
            <div className="section-card">
              ...
              <div style={{ marginTop: "16px" }}>
                ...
                {selectedConsultation?.medicalRecords?.length ? (
                  selectedConsultation.medicalRecords.map((record) => (
                    <div key={record.recordId} className="alert alert-info">
                      <div style={{ width: "100%" }}>
                        <p style={{ margin: 0, fontWeight: 600 }}>{record.fileName}</p>
                        <p style={{ margin: "4px 0" }}>{record.fileSummary}</p>
                        <small>{record.fileType} • {formatApiDateTime(record.uploadedAt)}</small>
                        <div style={{ marginTop: "8px" }}>
                          <button className="btn btn-secondary btn-sm" onClick={() => handleDownloadRecord(record)}>
                            Download
                          </button>
                        </div>
                      </div>
                  ...
                  <p style={{ color: "#6b7280" }}>No records in this consultation yet.</p>
                )}
              </div>
            </div>
          )}
